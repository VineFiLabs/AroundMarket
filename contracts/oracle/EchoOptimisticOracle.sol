...
contract EchoOptimisticOracle is Ownable, ReentrancyGuard, IEchoOptimisticOracle {
    ...
    uint64 public constant MINRANDOM = 1000000000000;
    uint64 public constant MAXRANDOM = 9999999999999;
    ...
    uint256 public TroubleMakingTime = 666;
    ...
    mapping(address => bool) public troublemaker;
    ...
    function initialize(address _aroundMarket, address _aroundPoolFactory) external onlyOwner {
    ...
    function setTroublemaker(address user, bool state) external onlyOwner {
        troublemaker[user] = state;
    }
    ...
    function registProvider() external { 
        ...
        if(whitelist[msg.sender] == false) {
            value = registFee * 10 ** _getUSDCDecimals();
            IERC20(usdc).safeTransferFrom(msg.sender, address(this), value);
        }
        dataProviderInfo[msg.sender].valid = true;
        dataProviderInfo[msg.sender].latestSubmitTime = uint64(block.timestamp);
        dataProviderInfo[msg.sender].depositeAmount += uint128(value);
        emit RegisterProvider(msg.sender);
    }

    function exit() external {
        require(dataProviderInfo[msg.sender].valid, "Not a challenger");
        require(whitelist[msg.sender] == false, "Whitelist stop");
        require(block.timestamp >= dataProviderInfo[msg.sender].latestSubmitTime + 7 days);
        uint256 depositeAmount = dataProviderInfo[msg.sender].depositeAmount;
        delete dataProviderInfo[msg.sender];
        IERC20(usdc).safeTransferFrom(msg.sender, address(this), depositeAmount);
        emit ExitProvider(msg.sender);
    }

    function disruptRandom(uint256 id, uint64 number) external {
        require(troublemaker[msg.sender], "Non troublemaker");
        require(block.timestamp <= oracleInfo[id].updateTime + TroubleMakingTime, "Finished");
        uint64 beforeRandom = oracleInfo[id].randomNumber;
        oracleInfo[id].randomNumber = beforeRandom / 2 + number;
    }

    function injectQuest(uint256 id, string calldata thisQuest) external {
        require(msg.sender == aroundPoolFactory, "Non aroundPoolFactory");
    ...
    function submitData(
        ...
        require(randomNumber >= MINRANDOM && randomNumber <= MAXRANDOM, "Invalid random numner");
    ...
    function getDataProviderInfo(address addr) external view returns (DataProviderInfo memory) {
        return dataProviderInfo[addr];
    }

    function getOnlyEventState(uint256 id) external view returns (EventState newEventState) {
        newEventState = oracleInfo[id].eventState;
    }
